...
export const purchasegroupSlice = createSlice({
    name: "purchasegroup",
    initialState,
    reducers: {
      ...
      partsreturnListFetched : (state, action) => {
        const { totalCount, entities } = action.payload;
        state.listLoading = false;
        state.error = null;
        state.partsreturnList = entities;
      ...
      deletePartsReturnListItem: (state, action) => {
        state.parstreturnList.splice(state.parstreturnList.findIndex((entity) => entity.id === action.payload), 1);
