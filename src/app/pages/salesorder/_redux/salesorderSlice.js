...
export const salesorderSlice = createSlice({
    ...
    reducers: {
      ...
      salesorderListFetched : (state, action) => {
        const { totalCount, entities } = action.payload;
        state.listLoading = false;
        state.error = null;
        state.salesorderList = entities;
      ...
      deleteSalesOrderListItem: (state, action) => {
        state.salesorderList.splice(state.salesorderList.findIndex((entity) => entity.id === action.payload), 1);
